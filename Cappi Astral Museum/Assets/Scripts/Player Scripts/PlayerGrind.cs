...
public class PlayerGrind : MonoBehaviour
{
    ...
    [Header("Scripts")]
    [SerializeField] public RailScript currentRailScript;
    ...
    void CalculateAndSetRailPosition()
    {
        ...
        transform.position = Vector3.Slerp(transform.position, splinePoint + (transform.up * heightOffset), 0.1f);
    }
    void ThrowOffRail()
    {
        ...
        pl.transformVelocity.y = Mathf.Sqrt(pl.jump * -2f * pl.gravity);
        pl.isGrounded = false;
        pl.state = "jump";
        pl.railFX.Stop();
        railDelay = 10;
